...
- `card.glow`：播放卡牌发光或强调动画。
- `card.move`：移动卡牌到指定位置或区域。
- `card.rename`：卡牌更名，例如 `??? -> 哈维`。
- `card.transform`：把一张卡转换成另一张卡，例如临时提示卡转为正式地点卡。
...
type GameCommand =
  ...
  | { type: 'card.glow'; cardInstanceId?: string; cardId?: string; durationMs?: number }
  ...
  | { type: 'card.transform'; fromCardId: string; toCardId: string; animation?: CardAnimation }
  ...
  sequence: [
    {
      ...
      results: [
        ...
        { type: 'card.glow', cardId: 'protagonist', durationMs: 800 },
...
- 卡牌交互结果可以通过数据触发卡牌溶解、消失、发光、移动、改名、生成新卡和剧情跳转。
- 树交互可以触发发现黑日的剧情。
- 哈维出现，并到达第一版计划终点。
